...
const registerCandidat = () => {
  ...
  store.dispatch('signUpCandidat', user)
    .then((response) => {
...
/*const registerCandidat = () => {
  axios.post('http://127.0.0.1:8000/api/register/signUpCondidat',
    {
      firstName : firstName.value,
      lastName : lastName.value,
      email : email.value,
...
<template>
  <AuthLayout :imageURL="imageURL" :User="'Candidat'" :pageName="'Signup'">
    <form  class="space-y-4" action="" @submit.prevent="registerCandidat">
      ...
      <div>
        ...
        <input type="password" placeholder="Confirm Password"  v-model="password_confirmation" id="password_confiramtion"
