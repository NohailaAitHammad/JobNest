...
@section('content')
    <div class="max-w-6xl mx-auto px-8 py-12">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-bold text-purple-800">My Proposals</h1>
            <a href="{{ route('candidat.dashboard') }}" class="text-sm text-purple-700 hover:underline flex items-center gap-1">
        ...
        <div class="space-y-4">
            @forelse($propositions as $prop)
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-purple-100 flex justify-between items-center hover:shadow-md transition-all">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">
                                {{ $prop->recruteur->firstName }} {{ $prop->recruteur->lastName }}
                                <span class="mx-2">•</span>
                                {{ $prop->recruteur->profileRecruteur->entreprise->nom ?? 'Company' }}
                            </p>
                    ...
                    <a href="{{ route('candidats.propositions.show', $prop->id) }}" class="px-5 py-2 bg-purple-700 text-white rounded-lg text-sm hover:bg-purple-800 transition-colors">
                        View Details
                    </a>
                </div>
            @empty
                <div class="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-purple-200">
                    <i class="fas fa-envelope-open text-purple-300 text-5xl mb-4"></i>
